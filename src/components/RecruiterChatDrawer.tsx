...
export const RecruiterChatDrawer: React.FC<RecruiterChatDrawerProps> = ({
  ...
  const quickPrompts = candidates.length > 1
    ? [
        `Why is ${candidates[0]?.candidate.name} ranked higher than ${candidates[1]?.candidate.name}?`,
        'Which candidate is best for backend engineering?',
        'Does any candidate have production React experience?',
    ...
    : candidates.length === 1
    ? [
        `Evaluate ${candidates[0]?.candidate.name}'s key strengths and missing skills.`,
        'What interview questions should I ask this candidate?',
      ]
    : [
        'What are the core technical requirements for this role?',
        'What skills would make an ideal candidate profile?',
      ];
